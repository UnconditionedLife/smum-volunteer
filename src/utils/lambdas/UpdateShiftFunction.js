...
export const handler = async (event) => {
    const TABLE_NAME = event.stageVariables?.shiftLogsTable ?? "dev-SMUM_ShiftLogs";
    ...
    try {
        ...
        if (isDeleted !== undefined) isDeleted = (isDeleted === null ? null : Boolean(isDeleted));
        ...
        const considerBoolean = (attr, val) => {
            if (val === undefined) return;
            if (val === null) return addRemove(attr);
            addSet(attr, { BOOL: val });              // store as Boolean
        };
        ...
        considerBoolean("isDeleted", isDeleted);
        ...
        const resp = {
            ...
            isDeleted: Attributes?.isDeleted?.BOOL
        };
